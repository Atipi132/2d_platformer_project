...
class Ennemy(NonPlayableCharacter):
    ...
    def player_interaction(self):
        ...
        if self.player.attacking and self.facing_right != self.player.facing_right and self.player.attack_position[0] >= self.rect.left and max(self.player.attack_position[1], self.rect.centery) - min(self.player.attack_position[1], self.rect.centery) <= 20 and (self.player.facing_right and self.player.attack_position[0] >= self.rect.left or not self.player.facing_right and self.player.attack_position[0] <= self.rect.right):
            self.dead = True
            self.frame_index = 0
            self.state = "Death"
